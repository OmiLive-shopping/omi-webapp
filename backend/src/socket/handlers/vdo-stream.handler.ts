...
const vdoMediaEventSchema = z.object({
  streamId: z.string().uuid(),
  action: z.enum(['audioMuted', 'audioUnmuted', 'videoHidden', 'videoShown', 'screenShareStarted', 'screenShareEnded']),
  timestamp: z.string().datetime(),
});
...
export class VdoStreamHandler {
  ...
  handleVdoRecordingEvent = async (socket: SocketWithAuth, data: any) => {
    try {
      ...
      this.socketServer.emitToRoom(`stream:${validated.streamId}:moderators`, 'vdo:recording:event', {
        action: validated.action,
   ...
   */
  handleGetVdoAnalytics = async (socket: SocketWithAuth, data: { streamId: string; period?: string }) => {
    try {
  ...
  private async aggregateAnalytics(streamId: string, stats: any) {
    ...
    for (const period of periods) {
      const periodData = filteredAnalytics.filter((a: any) => 
        a.timestamp.getTime() > now - period.duration
      );
